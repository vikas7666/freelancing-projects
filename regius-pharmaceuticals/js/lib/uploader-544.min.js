...
return data;}}});}));(function(factory){'use strict';if(typeof define==='function'&&define.amd){define(['jquery','./jquery.fileupload-process'],factory);}else if(typeof exports==='object'){factory(require('jquery'));}else{factory(window.jQuery);}}(function($){'use strict';$.blueimp.fileupload.prototype.options.processQueue.push({action:'validate',always:true,acceptFileTypes:'@',maxFileSize:'@',maxFileSizeArray:'@',minFileSize:'@',maxNumberOfFiles:'@',disabled:'@disableValidation'});$.widget('blueimp.fileupload',$.blueimp.fileupload,{options:{getNumberOfFiles:$.noop,messages:{maxNumberOfFiles:'Maximum number of files exceeded',acceptFileTypes:'File type not allowed',maxFileSize:'File is too large',minFileSize:'File is too small'}},processActions:{validate:function(data,options){if(options.disabled){return data;}
var dfd=$.Deferred(),settings=this.options,file=data.files[data.index],fileSize;if(options.minFileSize||options.maxFileSize){fileSize=file.size;}
var ext=file.name.substr(file.name.lastIndexOf('.')+1).toLowerCase();if($.type(options.maxNumberOfFiles)==='number'&&(settings.getNumberOfFiles()||0)+data.files.length>options.maxNumberOfFiles){file.error=settings.i18n('maxNumberOfFiles');}else if(options.acceptFileTypes&&!(options.acceptFileTypes.test(file.type)||options.acceptFileTypes.test(file.name))){file.error=settings.i18n('acceptFileTypes');}else if($.type(fileSize)==='number'&&fileSize<options.minFileSize){file.error=settings.i18n('minFileSize');}else{if(options.maxFileSizeArray!==undefined&&options.maxFileSizeArray[ext]){if(fileSize>options.maxFileSizeArray[ext]){file.error=settings.i18n('maxFileSize');}else{delete file.error;}}else if(fileSize>options.maxFileSize){file.error=settings.i18n('maxFileSize');}else{delete file.error;}}
if(file.error||data.files.error){data.files.error=true;dfd.rejectWith(this,[data]);}else{dfd.resolveWith(this,[data]);}
return dfd.promise();}}});}));$(document).ready(function(){$(document).on('click','.fileinput-button a',function(){$(this).siblings('input:file').click();preview_area=$(this).attr('id');var note=$(".note");note.html('');});$(document).on('click','table .files img, table .files canvas',function(){$(this).closest('table').siblings('.fileinput-button').find('a').click();});if($('#files_namespace').length){$("a").each(function(){if($(this).attr('href')&&$(this).attr('href').indexOf('javascript:')<0&&$(this).attr('data-dismiss')!='modal'&&$(this).attr('id')!='tl-confirm-submit'){if($(this).parents("#tl-files-grid").length<=0){$(this).attr('data-link','link');}}});}
$(document).on('click','[data-link="link"]',function(){myportal.app.leavingFromNormalLink=true;if(myportal.app.isUploading===true){myportal.app.confirmLeavePage("There is an active file upload transfer","Are you sure you want to leave this page? File upload will stop and you will lose all uploaded data",function(el){window.location=el.attr('href')},$(this));return false;}});$(window).bind("beforeunload",function(event){if(myportal.app.leavingFromNormalLink!==true){if(myportal.app.isUploading===true){return"There is an active file upload transfer";}}});$(document).on('click','.modal-preview',function(event){event.preventDefault();var action=$(this).attr('tl-action');if(typeof(action)!='undefined'){parameters={type:'GET',url:action,success:function(data){$("#preview_modal").empty().append(data);$("#preview_modal").modal();$('#tl-loading-pane').hide();}};myportal.app.ajax(action,parameters);}});});function disableSubmit(){$("input[type='submit']").prop('disabled',true);$("input[type='submit']").siblings('a.dropdown-toggle').addClass('disabled');}
function enableSubmit(){$("input[type='submit']").prop('disabled',false);$("input[type='submit']").siblings('a.dropdown-toggle').removeClass('disabled');}
function enableCrop(e,data,cropOptions,canvasSize){var id=$(e.target).attr('id');var file=data.files[0];var reader=new FileReader();reader.onload=function(evt){var $img=$('<img>',{src:evt.target.result});var original_width,original_height;var $crop_x=$('#'+cropOptions.inputPrefixName+'_crop_x');var $crop_y=$('#'+cropOptions.inputPrefixName+'_crop_y');var $crop_width=$('#'+cropOptions.inputPrefixName+'_crop_width');var $crop_height=$('#'+cropOptions.inputPrefixName+'_crop_height');var $crop_img_width=$('#'+cropOptions.inputPrefixName+'_crop_img_width');var $crop_img_height=$('#'+cropOptions.inputPrefixName+'_crop_img_height');var cropModalID=cropOptions.cropModalID?"#tl-crop-modal-"+cropOptions.cropModalID:"#tl-crop-modal";$(cropModalID+' .modal-body > .tl-crop-area').html($img);$img.on('load',function(){$(this).Jcrop({onChange:function(coords){if(typeof original_width==='undefined'){original_width=$img.width();}
if(typeof original_height==='undefined'){original_height=$img.height();}
if(coords.x>=0){$crop_x.val(coords.x);}
else{$crop_x.val(0);}
if(coords.y>=0){$crop_y.val(coords.y);}
else{$crop_y.val(0);}
if(coords.w<=original_width){$crop_width.val(coords.w);}
else{$crop_width.val(original_width);}
if(coords.h<=original_height){$crop_height.val(coords.h);}
else{$crop_height.val(original_height);}},onSelect:function(coords){if(typeof original_width==='undefined'){original_width=$img.width();}
if(typeof original_height==='undefined'){original_height=$img.height();}
if(coords.x>=0){$crop_x.val(coords.x);}
else{$crop_x.val(0);}
if(coords.y>=0){$crop_y.val(coords.y);}
...
if(coords.h<=original_height){$crop_height.val(coords.h);}
else{$crop_height.val(original_height);}},onRelease:function(){this.setOptions({setSelect:[0,0,$img.width(),$img.height()]});},bgFade:true,bgOpacity:.5,aspectRatio:cropOptions.selectionMinWidth / cropOptions.selectionMinHeight,minSize:[cropOptions.selectionMinWidth,cropOptions.selectionMinHeight],boxWidth:cropOptions.canvasMaxWidth,boxHeight:cropOptions.canvasMaxWidth},function(){window.jcrop_api=this;$crop_img_width.val($img.width());$crop_img_height.val($img.height());$crop_x.val('');$crop_y.val('');$crop_width.val('');$crop_height.val('');this.setSelect([0,0,$img.width(),$img.height()]);function cropImage(){if(!$crop_width.val()||!$crop_height.val()){_coords=window.jcrop_api.tellSelect();if(typeof original_width==='undefined'){original_width=$img.width();}
if(typeof original_height==='undefined'){original_height=$img.height();}
if(_coords.x>=0){$crop_x.val(_coords.x);}
else{$crop_x.val(0);}
if(_coords.y>=0){$crop_y.val(_coords.y);}
else{$crop_y.val(0);}
if(_coords.w<=original_width){$crop_width.val(_coords.w);}
else{$crop_width.val(original_width);}
if(_coords.h<=original_height){$crop_height.val(_coords.h);}
else{$crop_height.val(original_height);}}
$(cropModalID).modal('hide');var $canvas=$('#'+id).find('canvas');var scale=Math.min(cropOptions.selectionMaxWidth / parseInt($crop_width.val()),cropOptions.selectionMaxHeight / parseInt($crop_height.val()))
var new_canvas_width=(parseInt($crop_width.val())*scale<=canvasSize.width&&parseInt($crop_width.val())*scale>0)?parseInt($crop_width.val())*scale:canvasSize.width;var new_canvas_height=(parseInt($crop_height.val())*scale<=canvasSize.height&&parseInt($crop_height.val())*scale>0)?parseInt($crop_height.val())*scale:canvasSize.height;$canvas.attr('width',new_canvas_width);$canvas.attr('height',new_canvas_height);var destCtx=$canvas.get(0).getContext('2d');destCtx.drawImage($(cropModalID).find('.tl-crop-area img').get(0),$crop_x.val()||0,$crop_y.val()||0,$crop_width.val()||$crop_img_width,$crop_height.val()||$crop_img_height,0,0,new_canvas_width,new_canvas_height);}
if($img.width()<cropOptions.selectionMinWidth||$img.height()<cropOptions.selectionMinHeight){this.destroy();$('.tl-crop-switch').hide();if(cropOptions.hasOwnProperty('autoUploadAfterCrop')&&cropOptions.autoUploadAfterCrop==true){data.submit();}
return;}
if(cropOptions.cropOnLoad){cropImage();}
if(cropOptions.hasOwnProperty('autoOpenCrop')&&cropOptions.autoOpenCrop==true){$(cropModalID).modal({'backdrop':'static','keyboard':false});}
$('#'+id).find('.tl-crop-switch').off('click').click(function(){if($crop_x.val()==''){$crop_x.val(0);}
if($crop_y.val()==''){$crop_y.val(0);}
if($crop_width.val()==''){$crop_width.val(cropOptions.selectionMinWidth);}
if($crop_height.val()==''){$crop_height.val(cropOptions.selectionMinHeight);}
$(cropModalID).modal({'backdrop':'static','keyboard':false});});$(cropModalID).find('.tl-process-crop').off('click').click(function(){cropImage();if(cropOptions.hasOwnProperty('autoUploadAfterCrop')&&cropOptions.autoUploadAfterCrop==true){data.submit();}});$(cropModalID).find('.tl-close-crop-modal').off('click').click(function(){$(cropModalID).modal('hide');$crop_x.val('');$crop_y.val('');$crop_width.val('');$crop_height.val('');$crop_img_width.val('');$crop_img_height.val('');});})});};reader.readAsDataURL(file);}