...
local[prop.idx]=value;return this[fn](local);};});});each(stepHooks,function(i,hook){jQuery.cssHooks[hook]={set:function(elem,value){var parsed,backgroundColor,curElem;if(jQuery.type(value)!=='string'||(parsed=stringParse(value)))
{value=color(parsed||value);if(!support.rgba&&value._rgba[3]!==1){curElem=hook==="backgroundColor"?elem.parentNode:elem;do{backgroundColor=jQuery.curCSS(curElem,"backgroundColor");}while((backgroundColor===""||backgroundColor==="transparent")&&(curElem=curElem.parentNode)&&curElem.style);value=value.blend(backgroundColor&&backgroundColor!=="transparent"?backgroundColor:"_default");}
...
$img.bind('load.jcloader',completeCheck).bind('error.jcloader',function(e){$img.unbind('.jcloader');if($.isFunction(error))error.call(img);});if(img.complete&&$.isFunction(success)){$img.unbind('.jcloader');success.call(img);}};$.Jcrop.defaults={allowSelect:true,allowMove:true,allowResize:true,trackDocument:true,baseClass:'jcrop',addClass:null,bgColor:'black',bgOpacity:0.6,bgFade:false,borderOpacity:0.4,handleOpacity:0.5,handleSize:null,aspectRatio:0,keySupport:true,createHandles:['n','s','e','w','nw','ne','se','sw'],createDragbars:['n','s','e','w'],createBorders:['n','s','e','w'],drawBorders:true,dragEdges:true,fixedSupport:true,touchSupport:null,shade:null,boxWidth:0,boxHeight:0,boundary:2,fadeTime:400,animationDelay:20,swingSpeed:3,minSelect:[0,0],maxSize:[0,0],minSize:[0,0],onChange:function(){},onSelect:function(){},onDblClick:function(){},onRelease:function(){}};}(jQuery));(function($){window.URL=window.webkitURL||window.URL;function dataURItoBlob(dataURI,callback){var mimeString=dataURI.split(',')[0].split(':')[1].split(';')[0];var binary=atob(dataURI.split(',')[1]);var mimeString=dataURI.split(',')[0].split(':')[1].split(';')[0];var array=[];for(var i=0;i<binary.length;i++){array.push(binary.charCodeAt(i));}
return new Blob([new Uint8Array(array)],{type:mimeString});}
var methods={init:function(options){var self=this;var cropper={};self.options={boxWidth:600,boxHeight:300,height:100,mimeType:'image/png',sourceImageSelector:'img.source',previewContainerSelector:'div.preview-container',showPreview:true,width:100};var updatePreview=function(coords){self.currentCrop=coords;var context=self.preview.get(0).getContext('2d');if(self.currentCrop.w>0&&self.currentCrop.h>0){context.drawImage(self.source.get(0),self.currentCrop.x,self.currentCrop.y,self.currentCrop.w,self.currentCrop.h,0,0,self.options.width,self.options.height);}};self.setupCropper=function(){self.source.Jcrop({onChange:updatePreview,onSelect:updatePreview,aspectRatio:self.options.width / self.options.height,boxWidth:self.options.boxWidth,boxHeight:self.options.boxHeight});};self.loadImage=function(file){if(!window.URL){self.trigger('error.jcrop-canvas','Please use Chrome 10+ or FF 4+ to '+'use the image cropper.');return;}
var jcropHolder=$('.jcrop-holder');if(jcropHolder.length>0){$.Jcrop(self.source).destroy();jcropHolder.remove();}
self.source.unbind('load');self.source.load(function(e){window.URL.revokeObjectURL(self.source.src);self.setupCropper();self.source.show();if(self.options.showPreview===true){self.previewContainer.show();}else{self.previewContainer.hide();}
self.trigger('load');});self.source.attr('src',window.URL.createObjectURL(file));};$.extend(self.options,options);self.bind('dragover',function(e){e.preventDefault();return true;});self.bind('drop',function(e){e.preventDefault();self.loadImage(e.originalEvent.dataTransfer.files[0]);});self.source=$(self.options.sourceImageSelector);self.previewContainer=$(self.options.previewContainerSelector);self.previewContainer.html('<canvas class="preview"></canvas>');self.preview=self.previewContainer.find('.preview');self.preview.attr('width',self.options.width).attr('height',self.options.height);self.find('input[type=file]').change(function(){if(this.files.length>0){self.loadImage(this.files[0]);}});self.previewContainer.css('height',self.options.height).css('width',self.options.width);return this;},rotate:function(){var self=this;var canvas=$('<canvas></canvas>');var w=self.source.height();var h=self.source.width();canvas.attr('width',w);canvas.attr('height',h);canvas.hide();self.previewContainer.after(canvas);var context=canvas.get(0).getContext('2d');context.translate(w,0);context.rotate(Math.PI / 2);context.drawImage(self.source.get(0),0,0);self.loadImage(dataURItoBlob(canvas.get(0).toDataURL('image/png')));canvas.remove();},createCrop:function(){var self=this;if(!self.currentCrop){return null;}
var imgData=self.preview.get(0).toDataURL('image/png');return imgData.split(";base64,")[1];}};$.fn.jcropCanvas=function(method){if(methods[method]){return methods[method].apply(this,Array.prototype.slice.call(arguments,1));}
else if(typeof(method==='object')||!method){return methods.init.apply(this,arguments);}
else{$.error('Method '+method+' does not exist on jQuery.jcropCanvas.');}};})(jQuery);